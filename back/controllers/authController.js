...
module.exports.DietPlan = async (req, res) => {
  ...
  async function CreateDietPlan(
    ...
    let breakfast_counter=1;
    let lunch_counter=1
    let dinner_counter=1
    let remaining_counter= 0

    while (breakfastCalories>69 && breakfast_counter<arrfavbreakfast.length) {
      ...
      arrfavbreakfast.forEach((foodObject)=>{
        ...
        if(foodObject.max_serving?foodObject.max_serving-foodObject.preferred_serving*length>0:true){
          ...
          arrfavbreakfast.forEach((arrfavbreakfast_item)=>{
            const length2=dietplan.breakfast.filter((breakfastObject)=>breakfastObject.Food_id===arrfavbreakfast_item.Food_id).length
            if(breakfastCalories-arrfavbreakfast_item.food_calories_per_preferred_serving>0&&arrfavbreakfast_item.max_serving?arrfavbreakfast_item.max_serving-arrfavbreakfast_item.preferred_serving*length2>0:true){
            inc_counter=false
          }})
         if (inc_counter) {
          breakfast_counter++;
    ...
    while (lunchCalories>69 && lunch_counter<arrfavlunch.length) {
      ...
      arrfavlunch.forEach((foodObject)=>{
        ...
        if(foodObject.max_serving?foodObject.max_serving-foodObject.preferred_serving*length>0:true){
          ...
          arrfavlunch.forEach((arrfavlunch_item)=>{
            const length2=dietplan.lunch.filter((lunchObject)=>lunchObject.Food_id===arrfavlunch_item.Food_id).length
            if(lunchCalories-arrfavlunch_item.food_calories_per_preferred_serving>0&&arrfavlunch_item.max_serving?arrfavlunch_item.max_serving-arrfavlunch_item.preferred_serving*length2>0:true){
            inc_counter=false
          }})
         if (inc_counter) {
          lunch_counter++;
    ...
    while (dinnerCalories>69 && dinner_counter<arrfavdinner.length) {
      let inc_counter = true;
      arrfavdinner.forEach((foodObject)=>{
        ...
        if(foodObject.max_serving?foodObject.max_serving-foodObject.preferred_serving*length>0:true){
          ...
          arrfavdinner.forEach((arrfavdinner_item)=>{
            const length2=dietplan.dinner.filter((dinnerObject)=>dinnerObject.Food_id===arrfavdinner_item.Food_id).length
            if(dinnerCalories-arrfavdinner_item.food_calories_per_preferred_serving>0&&arrfavdinner_item.max_serving?arrfavdinner_item.max_serving-arrfavdinner_item.preferred_serving*length2>0:true){
            inc_counter=false
          }})
         if (inc_counter) {
          dinner_counter++;
    ...
    dietplan.reamaining=breakfastCalories+lunchCalories+dinnerCalories;
     ...
     if(dietplan.reamaining >= 100){
      if(breakfastCalories>lunchCalories && breakfastCalories > dinnerCalories){

        breakfast_counter=0;
        while (dietplan.reamaining>69 && breakfast_counter<arrfavbreakfast.length) {
          arrfavbreakfast.forEach((foodObject)=>{
              dietplan.breakfast.push(foodObject);
              dietplan.reamaining -= foodObject.food_calories_per_preferred_serving;
          })
        }

      }else if(lunchCalories > breakfastCalories&&lunchCalories>dinnerCalories){
        lunch_counter=0;
        while (dietplan.reamaining>69 && lunch_counter<arrfavlunch.length) {
          arrfavlunch.forEach((foodObject)=>{
      ...
      }else if(dinnerCalories > breakfastCalories &&dinnerCalories>lunchCalories){
        dinner_counter=0;
        while (dietplan.reamaining>69 && dinner_counter<arrfavdinner.length) {
          let inc_counter = true;
          arrfavdinner.forEach((foodObject)=>{
              dietplan.dinner.push(foodObject);
              dietplan.reamaining -= foodObject.food_calories_per_preferred_serving;
